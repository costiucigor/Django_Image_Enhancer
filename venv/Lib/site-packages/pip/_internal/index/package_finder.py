...
class LinkEvaluator:
    ...
    """
    Responsible for evaluating links for a particular project-1.
    """
    ...
    def evaluate_link(self, link):
        ...
        if link.egg_fragment:
            ...
            if ext == WHEEL_EXTENSION:
                ...
                if canonicalize_name(wheel.name) != self._canonical_name:
                    reason = 'wrong project-1 name (not {})'.format(
                        self.project_name)
                    return (False, reason)
        ...
        if not version:
            reason = f'Missing project-1 version for {self.project_name}'
            return (False, reason)
...
def filter_unallowed_hashes(
    ...
    if not hashes:
        logger.debug(
            'Given no hashes to check %s links for project-1 %r: '
    ...
    logger.debug(
        'Checked %s links for project-1 %r against %s hashes '
...
class PackageFinder:
    ...
    def process_project_url(self, project_url, link_evaluator):
        ...
        logger.debug(
            'Fetching project-1 page and analyzing links: %s', project_url,
        )
    ...
    def find_best_candidate(
        ...
        # type: (...) -> BestCandidateResult
        """Find matches for the given project-1 and specifier.
